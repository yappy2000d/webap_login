...
def normalize(img: np.ndarray, w: int, h: int, mean: float, std: float) -> np.ndarray:
    ...
    img = img / 255.0           # [captcha_utils](https://github.com/NKUST-ITC/NKUST-AP-Flutter/blob/eb77232a1af1365da5e4610c145798f092c41933/lib/utils/captcha_utils.dart#L118)
...
for i in range(digitsCount):
    ...
    interpreter.set_tensor(input_details[0]['index'], img_normalized[np.newaxis, :, :, np.newaxis].astype(np.float32))
